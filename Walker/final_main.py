...
def initialize_population(size):
    """Initialize the population of robots."""
    population = []
...
def extract_features(robot):
    """Extract features (active ratio, actuator ratio) from the robot."""
...
# ----- Map-Elites Grid -----
def create_map_grid():
    """Create a new empty map grid."""
    return np.full((MAP_RESOLUTION, MAP_RESOLUTION), None, dtype=object)

def update_map(map_grid, robot, reward):
    """Update the map with the robot's features and reward."""
    ...
    if map_grid[bin1, bin2] is None or reward > map_grid[bin1, bin2]['reward']:
        print(f"Updating bin {bin1},{bin2} with new robot (reward: {reward}).")
        map_grid[bin1, bin2] = {'robot': robot, 'reward': reward}

def pad_or_crop(robot, target_size=VOXEL_GRID_SIZE):
    """Pad or crop the robot to the target size."""
...
def mutate(robot):
    """Perform mutation on the robot, ensuring it remains connected."""
    mutated_robot = np.copy(robot)
    mutation_cache = []
    
    mutation_rate = np.random.randint(1, 3)  
...
def batch_train_robots(robot_list, args, model_save_dir, structure_save_dir):
    """Train robots in batches using PPO and return the results."""
    results = []
...
# ----- Evolutionary Loop -----
def evolve(args):
    """Main evolutionary loop to drive robot training and mutation."""
    ...
    for gen in range(GENERATIONS):
        ...
        for robot in tqdm(population):
            reward = run_ppo(
                ...
                env_name=args.env_name,
                model_save_dir=model_save_dir, 
                model_save_name=f"robot_{gen}",
            )
            update_map(map_grid, robot, reward)
        ...
        with open(f"{exp_dir}/generation_{gen+1}_archive.json", "w") as json_file:
            json.dump(all_results, json_file, indent=4)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--env-name", default="Walker-v0")
    parser.add_argument("--save-dir", default="final_try")
    parser.add_argument("--exp-name", default="final_trys")
    add_ppo_args(parser)
