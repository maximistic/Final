...
def save_policy(policy, save_path):
    """Save PPO policy to file."""
    policy.save(save_path)

def load_policy(load_path):
    """Load PPO policy from file."""
...
def evolve(args):
    ...
    os.makedirs(os.path.join(save_dir, "morphologies"), exist_ok=True)
    ...
    for gen in range(GENERATIONS):
        ...
        # Train robots
        robot_list = [(robot, (i, j)) for i in range(resolution) for j in range(resolution) if map_grid[i, j]]
        results = batch_train_robots(robot_list, args, os.path.join(save_dir, "models"), os.path.join(save_dir, "morphologies"), shared_policy)

        # Update map
        for robot, (i, j), fitness in results:
        ...
        # Save shared policy
        shared_policy_path = os.path.join(save_dir, "models", f"shared_policy_gen_{gen + 1}.zip")
        save_policy(shared_policy, shared_policy_path)
        ...
        save_json(gen_data, os.path.join(save_dir, "metrics", f"generation_{gen + 1}.json"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save-dir", default="draft_guide2")
