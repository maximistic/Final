...
def save_json(data, save_path):
    """Save JSON data with numpy compatibility."""
    def convert(obj):
...
def calculate_behavioral_diversity(features):
    """Calculate behavioral diversity as the mean pairwise distance between features."""
    return np.mean(squareform(pdist(features))) if len(features) > 1 else 0

def calculate_structural_diversity(robots):
    """Calculate structural diversity based on differences in robot morphology."""
    distances = []
...
def pad_or_crop(robot, target_size=VOXEL_GRID_SIZE):
    """Pad or crop a robot to match the target voxel grid size."""
...
def mutate_robot(robot):
    """Mutate robot morphology with validity checks."""
    while True:
        mutated = robot.copy()
...
def crossover_robots(robot_a, robot_b):
    """Perform multi-point crossover between two robots."""
    while True:
...
def evolve_population(map_grid):
    """Generate a new population of robots using mutation and crossover."""
    new_population = []
...
def batch_train_robots(robot_list, args, model_save_dir, structure_save_dir, shared_policy=None):
    """Train a batch of robots and return their performance."""
    results = []
...
def discretize_features(active_ratio, actuator_ratio):
    """Discretize robot features into grid bins."""
...
def update_map(map_grid, robot, fitness=0):
    """Update the map grid with a robot's features and fitness."""
...
def extract_features(robot):
    """Extract active and actuator ratios from a robot."""
    total_voxels = np.prod(robot.shape)
    active_voxels = np.sum(robot > 0)
    actuator_voxels = np.sum(robot == 3)
...
def update_shared_policy(shared_policy, best_robot, args, save_dir):
    """Update shared policy by fine-tuning."""
    try:
...
def save_generation_data(map_grid, generation, save_dir):
    """Save generation data, including the map grid and diversity metrics."""
    archive = {}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exp-name", default="sim_cooptimization3")
    add_ppo_args(parser)
